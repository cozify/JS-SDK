(function (global, factory) {
  ...
  ***************************************************************************** */
  ...
  function _objectSpread$5(target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i] != null ? arguments[i] : {}; if (i % 2) { ownKeys$6(source, true).forEach(function (key) { _defineProperty$2(target, key, source[key]); }); } else if (Object.getOwnPropertyDescriptors) { Object.defineProperties(target, Object.getOwnPropertyDescriptors(source)); } else { ownKeys$6(source).forEach(function (key) { Object.defineProperty(target, key, Object.getOwnPropertyDescriptor(source, key)); }); } } return target; }
  ...
  function defineFieldMap(config) {
    ...
    return mapValue(fieldMap, function (fieldConfig, fieldName) {
      ...
      return _objectSpread$5({}, fieldConfig, {
        name: fieldName,
  ...
  function KnownArgumentNames(context) {
    return _objectSpread$6({}, KnownArgumentNamesOnDirectives(context), {
  ...
  function ProvidedRequiredArguments(context) {
    return _objectSpread$7({}, ProvidedRequiredArgumentsOnDirectives(context), {
      Field: {
  ...
  var defKindToExtKind = (_defKindToExtKind = {}, _defineProperty$5(_defKindToExtKind, Kind.SCALAR_TYPE_DEFINITION, Kind.SCALAR_TYPE_EXTENSION), _defineProperty$5(_defKindToExtKind, Kind.OBJECT_TYPE_DEFINITION, Kind.OBJECT_TYPE_EXTENSION), _defineProperty$5(_defKindToExtKind, Kind.INTERFACE_TYPE_DEFINITION, Kind.INTERFACE_TYPE_EXTENSION), _defineProperty$5(_defKindToExtKind, Kind.UNION_TYPE_DEFINITION, Kind.UNION_TYPE_EXTENSION), _defineProperty$5(_defKindToExtKind, Kind.ENUM_TYPE_DEFINITION, Kind.ENUM_TYPE_EXTENSION), _defineProperty$5(_defKindToExtKind, Kind.INPUT_OBJECT_TYPE_DEFINITION, Kind.INPUT_OBJECT_TYPE_EXTENSION), _defKindToExtKind);
  ...
  var SubscriptionClient = (function () {
      ...
      SubscriptionClient.prototype.checkOperationOptions = function (options, handler) {
          ...
          if ((!isString_1.default(query) && !getOperationAST_1.getOperationAST(query, operationName)) ||
              ...
              (variables && !isObject_1$1.default(variables))) {
  ...
  */
  ...
  const GET_PLAN_DOCUMENTs_AND_NODES = src`
...
`;
  const INSERT_PLAN_DOCUMENT = src`
...
`;
  ...
  */
  ...
  const UPDATE_PLAN_DOCUMENT = src`
...
`;
  const REMOVE_PLAN_DOCUMENT = src`
...
`; // Nodes
  ...
  const GET_PLAN_DOCUMENT_NODES = src`
...
`;
  const SUBS_PLAN_DOCUMENT_NODES = src`
...
`;
  const INSERT_PLAN_DOCUMENT_NODE = src`
...
`;
  ...
  */
  ...
  const UPDATE_PLAN_DOCUMENT_NODE = src`
...
`;
  const REMOVE_PLAN_DOCUMENT_NODE = src`
...
`;
  ...
  */
  ...
  const reactSubscribePlans = createAsyncThunk('plans/subscribePlans', async (params, ThunkAPI) => {
    return new Promise((resolve, reject) => {
      if (isAuth(ThunkAPI.getState())) {
        const subsHandle = qqlClient.subscribe({
          query: SUBS_PLANS //variables: { },

        }).subscribe({
          next(data) {
            try {
              if (data && data.data && data.data.t_plan && data.data.t_plan.length > 0) {
                const results = normalize$1(data); // plansAdapter.setAll(ThunkAPI.getState(), results.plans)

                ThunkAPI.dispatch(plansState.actions.setPlansState(results.plans));
              } else {
                ThunkAPI.dispatch(plansState.actions.setPlansState({
                  plans: {}
                }));
              }
  ...
  */
  ...
  const reactSubscribePlanDocuments = createAsyncThunk('plans/subscribePlanDocuments', async (params, ThunkAPI) => {
    return new Promise((resolve, reject) => {
      if (isAuth(ThunkAPI.getState())) {
        const {
          planId
          ...
          next(data) {
            try {
              if (data && data.data && data.data.t_plan_document && data.data.t_plan_document.length > 0) {
                const results = normalize$1(data);
                ThunkAPI.dispatch(planDocumentsState.actions.setPlanDocumentsState(results.documents));
              } else {
                ThunkAPI.dispatch(planDocumentsState.actions.setPlanDocumentsState({
                  documents: {}
                }));
              }
  ...
  */
  ...
  const reactSubscribePlanDocumentNodes = createAsyncThunk('plans/subscribePlanDocumentNodes', async (params, ThunkAPI) => {
    return new Promise((resolve, reject) => {
      if (isAuth(ThunkAPI.getState())) {
        const {
          documentId
          ...
          next(data) {
            try {
              if (data && data.data && data.data.t_plan_document_node && data.data.t_plan_document_node.length > 0) {
                ...
                ThunkAPI.dispatch(planDocumentNodesState.actions.setPlanDocumentNodesState({
                  nodes: {}
                }));
              }
  ...
  const reactSelectPlanDocumentNodes = planDocumentNodesState.selectors.selectPlanDocumentNodes;
  const {
    actions: actions$9,
    reducer: reducer$9
  ...
  */
  ...
  function doPwLogin(email, password) {
    return new Promise((resolve, reject) => {
      send({
        ...
        if (response && isString_1$1(response)) {
          store.dispatch(userState.actions.setAuthKey(response));
  ...
  function pairingDevicesDeltaHandler(hubId, reset, pairingDevices) {
    ...
    pairingDevices.map(device => {
      const deviceProps = _objectSpread$8({}, device.status); // TODO: Start of remove when hub 'device.status.state.state'-bug fixed
      ...
      */
      ...
      device = _objectSpread$8({}, device, {}, deviceProps);
  ...
  **
  ...
  var _defineProperty$6 = defineProperty$1;
   ...
   */
  var baseSetToString = !_defineProperty$6 ? identity_1 : function(func, string) {
    return _defineProperty$6(func, 'toString', {
      'configurable': true,
      'enumerable': false,
    ...
    function simplifyNode(childNode) {
      ...
      for (let i = 0; i < childNode.childIds.length; i += 1) {
        ...
        if (nodes.locations[childNode.childIds[i]] && nodes.locations[childNode.childIds[i]].data) {
          nodesTree[i] = _objectSpread$9({}, nodes.locations[childNode.childIds[i]].data);
